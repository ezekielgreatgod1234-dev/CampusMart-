...
function SellerSettings({ unreadMessages = 0, profile = {} }) {
  ...
  const handleInstallApp = async () => {
    ...
    try {
      ...
      console.error("CampusMart install prompt error:", error);
    }
  };
  ...
  const handleContactSubmit = async () => {
    ...
    if (subject.length > 150) {
      setContactError("Subject must be 150 characters or less.");
      return;
    }

    if (message.length > 5000) {
      setContactError("Message must be 5000 characters or less.");
      return;
    }
    ...
    try {
      ...
      await addDoc(collection(db, "supportMessages"), supportMessage);
      ...
      >
        <div className="relative px-5 pt-19 lg:pt-5 pb-4 flex-shrink-0">
          ...
          <div className="flex items-center gap-3 pr-10">
            <div
              className="
                w-10 h-10 min-w-[40px] rounded-xl bg-[#006f2e]
                  ...
                  <div key={section.title}>
                    ...
                    <div className="space-y-1">
                      {section.items.map((item) => {
                        ...
                        const isInstallItem = item.action === "install";
                        ...
                        return (
                          <button
                            ...
                            disabled={isInstallItem && isInstalled}
                            className={`
                              w-full flex items-center justify-between gap-3
                              px-3 py-3 rounded-xl text-left transition
                              ${
                                active && !isInstallItem
                                  ? "bg-green-50 text-green-600"
                                  : isInstalled && isInstallItem
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div
                                className={`
                                  ...
                                  ${
                                    active || (isInstalled && isInstallItem)
                            ...
                            {isInstallItem && !isInstalled && (
                              <FiChevronRight size={16} className="text-green-500" />
                            )}
