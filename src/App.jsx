...
import Gigs from "./pages/customer/Gigs";
...
function InstalledAppSplash() {
  ...
  return (
    <div
      className={`fixed inset-0 z-[99999] flex min-h-screen items-center justify-center bg-white transition-opacity duration-1000 ${
...
function GuestRoute({ children, profile, profileResolved }) {
  ...
  const justRegistered =
    ...
    sessionStorage.getItem("campusmart_just_registered") === "1";
  ...
  if (
    profileLoading ||
    (firebaseUser && !profileResolved && !justRegistered && !isAuthPage)
  ) {
    return <LoadingScreen text="Checking your account..." />;
  }

  if (firebaseUser && !justRegistered && !isAuthPage) {
...
function AdminRoute({ children, profile, profileResolved }) {
  const { firebaseUser, profileLoading } = useAuth();
  ...
  if (!profileResolved) {
    return <LoadingScreen text="Loading your admin account..." />;
  }
...
function getMessageTimestamp(message) {
  ...
  if (typeof message.time === "string" && message.time.trim()) {
    const parsedTime = Date.parse(`1970-01-01 ${message.time.trim()}`);
...
async function formatConversation(conversationDoc, currentUserId) {
  ...
  const visibleMessages = conversationMessages.filter((message) => {
    const deletedFor = Array.isArray(message.deletedFor)
      ? message.deletedFor
      : [];
    if (deletedFor.includes(currentUserId)) return false;
...
function App() {
  ...
  useEffect(() => {
    ...
    const loadCurrentUserProfile = async () => {
      ...
      try {
        ...
        if (cancelled || currentRequest !== profileRequestId.current) return;
  ...
  const queueCustomerDataSave = ({
    ...
    pendingCustomerData.current = { nextCart, nextWishlist, nextOrders };
    ...
    customerDataSaveTimer.current = window.setTimeout(() => {
      ...
      pendingCustomerData.current = null;
      customerDataSaveTimer.current = null;
      if (dataToSave) writeCustomerData(dataToSave);
  ...
  const placeOrder = async (orderData) => {
    if (!orderData || !firebaseUser) return null;
        ...
        for (const item of sellerItems) {
          ...
          try {
            await updateDoc(doc(db, "products", String(productId)), {
              sales: increment(Number(item.quantity) || 1),
              updatedAt: serverTimestamp(),
            });
          } catch (salesErr) {
  ...
  const deleteMessages = async (conversationId, messageIds, deleteType) => {
    ...
    if (deleteType !== "me" && deleteType !== "everyone") return false;
    ...
    try {
      ...
      const result = await runTransaction(db, async (transaction) => {
        ...
        const existingMessages = Array.isArray(data.messages)
          ? data.messages
          : [];
        const selectedIds = new Set(messageIds.map((id) => String(id)));

        if (deleteType === "everyone") {
          const updatedMessages = existingMessages.filter((message) => {
            if (!selectedIds.has(String(message.id))) return true;
        ...
        const updatedMessages = existingMessages.map((message) => {
          if (!selectedIds.has(String(message.id))) return message;

          const deletedFor = Array.isArray(message.deletedFor)
            ? message.deletedFor
            : [];

          if (deletedFor.includes(firebaseUser.uid)) return message;
  ...
  const openSellerChat = async (product) => {
    if (!firebaseUser || !product) return false;

    const sellerId =
      product.sellerId || product.sellerUid || product.seller?.uid || "";

    if (!sellerId || String(sellerId) === String(firebaseUser.uid)) {
      return false;
    }
  ...
  // =======================================================
  // AUTH INITIALIZATION  ✅ FIXED – public paths stay open
  // =======================================================
  ...
  return (
    <>
      ...
      <Routes>
        <Route path="/app-start" element={<InstalledAppSplash />} />
        <Route path="/" element={<PwaAwareHome />} />
...
/>
        ...
        <Route
          ...
          element={
            <AdminRoute profile={profile} profileResolved={profileResolved}>
              <AdminSupportMessages profile={profile} />
            </AdminRoute>
          }
        />
